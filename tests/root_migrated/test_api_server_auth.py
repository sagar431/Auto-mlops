...
Run with: pytest tests/root_migrated/test_api_server_auth.py -v
"""
...
class TestSessionAccessControl:
    """Tests for API session ownership checks."""

    def test_session_owner_has_access(self):
        from api_server import require_session_access
        from security import session_ownership
        from security.middleware import CurrentUser

        session_ownership.register_session("session-owner-test", "user-1")
        user = CurrentUser(user_id="user-1", is_authenticated=True, auth_method="jwt")

        require_session_access(user, "session-owner-test")
        session_ownership.unregister_session("session-owner-test")
    ...
    def test_non_owner_is_denied(self):
        ...
        session_ownership.register_session("session-deny-test", "user-1")
        user = CurrentUser(user_id="user-2", is_authenticated=True, auth_method="jwt")

        with pytest.raises(AuthorizationError):
            require_session_access(user, "session-deny-test")

        session_ownership.unregister_session("session-deny-test")
    ...
    def test_admin_has_access(self):
        ...
        session_ownership.register_session("session-admin-test", "user-1")
        user = CurrentUser(
            user_id="admin-1",
        ...
        require_session_access(user, "session-admin-test")
        session_ownership.unregister_session("session-admin-test")
