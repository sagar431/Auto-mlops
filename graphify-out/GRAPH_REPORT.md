...
5. `AgentLoop` - 517 edges
6. `SessionRepository` - 437 edges
7. `ModelMetrics` - 411 edges
8. `AsyncSessionRepository` - 398 edges
9. `ValidationSeverity` - 394 edges
...
Cohesion: 0.29
Nodes (8): AddDataToDVCInput, ConfigureDVCRemoteInput, Configure DVC remote storage., Configure DVC remote storage., Configure DVC remote storage., Add data to DVC tracking., Configure DVC remote storage., Add data to DVC tracking.
...
- **Are the 512 inferred relationships involving `AuthenticationError` (e.g. with `SecurityConfig` and `APIKeyAuth`) actually correct?**
