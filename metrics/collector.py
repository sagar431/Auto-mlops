...
class MetricsCollector:
    ...
    def get_pipeline_metrics(self) -> PipelineMetrics:
        ...
        for tool_name, stats in sorted_tools:
            most_used.append(ToolUsageStats(
                tool_name=tool_name,
                invocations=stats.get('invocations', 0),
                success_count=stats.get('success', 0),
                failure_count=stats.get('failure', 0),
                avg_duration_ms=round(stats.get('total_duration', 0) / max(stats.get('invocations', 1), 1), 2)
            ))
    ...
    def get_metrics_summary(self) -> MetricsSummary:
        ...
        self._sessions_history.append(TimeSeriesDataPoint(timestamp=now, value=float(self._total_sessions)))
    ...
    def record_accuracy(self, accuracy: float, session_id: Optional[str] = None):
    ...
    def _log(self, level: str, source: str, message: str, session_id: Optional[str] = None, metadata: Dict = None):
    ...
    def log(self, level: str, source: str, message: str, session_id: Optional[str] = None, metadata: Dict = None):
    ...
    def generate_demo_data(self):
        ...
        for i in range(50):
            ...
            self._cpu_history.append(TimeSeriesDataPoint(
                timestamp=ts,
                value=random.uniform(20, 80)
            ))
            self._memory_history.append(TimeSeriesDataPoint(
                timestamp=ts,
                value=random.uniform(40, 70)
            ))
            self._sessions_history.append(TimeSeriesDataPoint(
                timestamp=ts,
                value=float(i * 2)
            ))
            if i > 30:
                self._accuracy_history.append(TimeSeriesDataPoint(
                    timestamp=ts,
